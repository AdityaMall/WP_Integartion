codeunit 50010 "WhatsApp Management"
{
    internal procedure SendMessage(PhoneNo: Text[30]; CountryCode: Code[10]; Message: Text[100])
    var
    ...
    begin
        if Country.Get(CountryCode) then begin
            if Country."Dial" = '' then
                exit;

            if CopyStr(PhoneNo, 2, StrLen(Country."Dial")) = Country."Dial" then
                PhoneNo := DelStr(PhoneNo, 1, StrLen(Country."Dial") + 1);
            PhoneNo := Format('+' + Country."Dial" + PhoneNo);
        end;
    end;
